#!/usr/bin/env python3
"""Generate index.html for the Chinese Poetry Learning App.
Uses <img> tags with object-fit:contain for proper image display.
"""
import json, sys

# Load poems
with open('./poems80.json', encoding='utf-8') as f:
    poems = json.load(f)

poems_json = json.dumps(poems, ensure_ascii=False)

# JavaScript: IIFE with all logic
js = r"""
(function() {
  ...
  // Use <img> tags - reliable across all browsers, supports object-fit for aspect ratio
  function imgHTML(po){
    if(!po.imgUrl)return'<div style="height:160px;width:100%;border-radius:12px;margin-bottom:.75rem;display:flex;align-items:center;justify-content:center;font-size:3rem;opacity:.2;color:#fff;background:'+gradFor(po)+'">'+iconEmoji(po)+'</div>';
    var url=po.imgUrl;
    var alt=e(po.poem);
    return'<div style="width:100%;border-radius:12px;margin-bottom:.75rem;overflow:hidden;background:'+gradFor(po)+';display:flex;align-items:center;justify-content:center;min-height:120px;max-height:280px"><img src="'+url+'" alt="'+alt+'" style="width:100%;max-height:280px;object-fit:contain;display:block;border-radius:8px" onerror="this.parentElement.style.display=\'none\'" /></div>';
  }
  ...
  function render(){
    ...
    if(state.v==='solar'||state.v==='fest'){
      ...
      var catBtns=(isSolar?SEASONS:FESTIVALS).map(function(c){
        ...
        var col=isSolar?SEASON_COLORS[SEASONS.indexOf(name)]:c.c;
        var grad=isSolar?SEASON_GRAD[name]:'linear-gradient(135deg,'+col+'88,'+col+'44)';
  ...
  window._aq=function(idx){
    ...
    state.exp=true;
    if(correct){
      state.conf=true;render();
      setTimeout(function(){state.conf=false;
